...
bool SpeedTest::ipInfo(IPInfo& info) {
    ...
    CURL url(SPEED_TEST_IP_INFO_API_URL);
    XFILE::CCurlFile curl;
    std::string strResponse;
    if (curl.Get(url.Get(), strResponse))
    {
      auto values = SpeedTest::parseQueryString(strResponse.c_str());
      mIpInfo = IPInfo();
...
CURLcode SpeedTest::httpPost(const std::string &url, const std::string &postdata, std::stringstream &os, CURL *handler, long timeout) {
...
CURL *SpeedTest::curl_setup(CURL *handler) {
    CURL* curl = handler == nullptr ? new CURL() : handler;
    if (curl){
...
bool SpeedTest::fetchServers(const std::string& url, std::vector<ServerInfo>& target, int &http_code) {
  ...
  XFILE::CCurlFile curl;
  std::string strResponse;
  if (!curl.Get(url1.Get(), strResponse))
  {
    return false;
  }
    size_t len = strResponse.size();
    auto *xmlbuff = (char*)calloc(len + 1, sizeof(char));
    if (!xmlbuff){
        std::cerr << "Unable to calloc" << std::endl;
        return false;
    }
    memcpy(xmlbuff, strResponse.c_str(), len);
